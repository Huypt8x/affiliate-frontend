...
export default function AffiliatePage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterTrending) {
      arr = arr.filter((c) => c.trending);
    }

    if (filterWeekly) {
      arr = arr.filter((c) => c.weekly);
    }
  ...
  const handleExplore = (category) => {
    // Mở trực tiếp trang chủ của program đầu tiên trong category
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((cat, idx) => (
          <CategoryCard
            ...
            onFilterTrending={() => setFilterTrending(true)}
            onFilterWeekly={() => setFilterWeekly(true)}
          />
        ))}
      </div>
    </div>
