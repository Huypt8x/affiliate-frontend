...
export default function AffiliateTable({ categories, filter, sort }) {
  ...
  const computeStats = (programs) => {
    ...
    const avgEpu = (programs.reduce((sum, p) => sum + parseNumber(p.epu), 0) / totalPrograms).toFixed(2);
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    // ✅ Lọc theo category, hỗ trợ cả categoryLabel và category
    if (filter !== "ALL") {
      data = data.filter((item) => item.categoryLabel === filter || item.category === filter);
    }
